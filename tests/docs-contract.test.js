...
test('project status separates published release from maintenance candidates', () => {
  ...
  assert.match(status, /Post-release verification on 2026-05-01 recorded npm versions as\s+`\["0\.1\.0","0\.1\.5","0\.1\.6"\]`/);
  assert.match(status, /Post-release verification on 2026-05-01 recorded the npm `latest` dist-tag as `0\.1\.6`/);
  assert.match(status, /GitHub release `v0\.1\.0` exists/);
  assert.match(status, /npm package `encourage-loop@0\.1\.5` is visible/);
  assert.match(status, /npm package `encourage-loop@0\.1\.6` is visible/);
  assert.match(status, /GitHub release `v0\.1\.5` exists/);
  assert.match(status, /GitHub release `v0\.1\.6` exists/);
  assert.match(status, /Remote tag `v0\.1\.5` points to `23281c46f1a97a780a7c59be4557aeb954d9fcf2`/);
  assert.match(status, /Remote tag `v0\.1\.6` points to `d54edc946fb7a20de56091c3275d3a15e9592a52`/);
  ...
  assert.doesNotMatch(status, /v0\.1\.6[^.\n]*has not been released/);
